...
def random_response_builder(wrap_num):
    ...
    start = int(((CRAP_SIZE) - size) / 2)     # set start point on half of the excess crap outside chosen size to slice out the center instead of from beginning
    slice_obj = slice(start, (start + size))  # create the slicer
    crap = CRAP_INIT[slice_obj]                # slice the crap
    craptastic = str.encode(open_close[wrap_num][0]) + crap + str.encode(open_close[wrap_num][1]) # wrap the crap in something nice
